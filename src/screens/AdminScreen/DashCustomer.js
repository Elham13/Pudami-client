...
function DashCustomer() {
  ...
  return (
    <div className=" w-auto md:w-full md:ml-14 flex flex-col justify-start items-start  mx-auto bg-transparent text-coolGray-800 text-sm md:text-base ">
      {loading || loadingO ? (
        <LoadingBox />
      ) : error || errorO ? (
                  ...
                  <td className="flex justify-start py-2 items-center space-x-2">
                    ...
                    >
                      {user.profile.img === "" ? (
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          class="h-7 w-7 ml-2 text-red-500 bg-red-200  rounded-full "
                          viewBox="0 0 20 20"
                          fill="currentColor"
                        >
                          <path
